...
CMainFrame::CMainFrame()
{
	...
	m_bXPMode = regMgr.GetProfileInt(_T("Settings"), _T("bXPMode"), FALSE);
...
{
	CXTPRegistryManager regMgr;
	regMgr.WriteProfileInt(_T("Settings"), _T("bXPMode"), m_bXPMode);
}
	...
	CCreateContext* pContext)
{
	...
	m_wndSplitter.SetSplitterStyle (XTP_SPLIT_DOTTRACKER);
...
void CMainFrame::InitializeAddressBar()
{
	...
	m_wndAddressBar.SetFont(&XTPAuxData().font);
...
void CMainFrame::OnViewXpmode()
{
	...
	CXTPCaptionButton& captBtn = m_pTreeView->m_wndCaption.GetCaptionButton();

	if (m_bXPMode)
	{
		captBtn.SetTheme(xtpControlThemeOfficeXP);
	}
	else
	{
		captBtn.SetTheme(xtpControlThemeDefault);
	}
