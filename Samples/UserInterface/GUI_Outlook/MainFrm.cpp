...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_UPDATE_COMMAND_UI_RANGE(ID_SKIN_LUNABLUE, ID_SKIN_DISABLE, OnUpdateSkin)
	...
	ON_MESSAGE(XTPWM_TASKPANEL_NOTIFY, OnShortcutLBoxNotify)
...
CMainFrame::CMainFrame()
{
	...
	m_bShowPreview = TRUE;
	m_pwndTreeCtrl = NULL;
	m_nFrameworkSkin = ID_SKIN_DISABLE;
...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT /*lpcs*/, CCreateContext* pContext)
{
	...
	m_wndSplitter2.SetSplitterStyle(XTP_SPLIT_NOFULLDRAG | XTP_SPLIT_NOSIZE | XTP_SPLIT_NOBORDER);
...
LRESULT CMainFrame::OnShortcutLBoxNotify(WPARAM wParam, LPARAM lParam)
{
	switch (wParam)
	{
	case XTP_TPN_CLICK:
...
void CMainFrame::UpdateButtonStyle(CXTPCaptionButton& btnCaption)
{
	...
	{
		btnCaption.SetTheme(xtpControlThemeOfficeXP);
	}
	else
	{
		btnCaption.SetTheme(xtpControlThemeOffice2000);
	}
}
...
void CMainFrame::SetTheme(XTPPaintTheme nTheme)
{
	...
	if ( m_nTheme == xtpThemeNativeWinXP)
	{
		m_wndCaption.SetTheme(xtpControlThemeOffice2003);
		m_pItemTreeView->m_wndCaption.SetTheme(new CXTPCaptionThemeNativeWinXP);
	}
	...
	{
		m_wndCaption.SetTheme(nControlsTheme);
		m_pItemTreeView->m_wndCaption.SetTheme(new CXTPCaptionThemeOffice2003);
	}
...
void CMainFrame::OnSkinChanged(UINT nID)
{
	...
	m_nFrameworkSkin = nID;
#endif
}
void CMainFrame::OnUpdateSkin(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(pCmdUI->m_nID==m_nFrameworkSkin);	
