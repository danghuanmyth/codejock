...
IMPLEMENT_DYNCREATE(CGUI_OutlookView, CXTPListView)

BEGIN_MESSAGE_MAP(CGUI_OutlookView, CXTPListView)
	//{{AFX_MSG_MAP(CGUI_OutlookView)
	ON_WM_CREATE()
	//}}AFX_MSG_MAP
	// Standard printing commands
	ON_COMMAND(ID_FILE_PRINT, CXTPListView::OnFilePrint)
	ON_COMMAND(ID_FILE_PRINT_DIRECT, CXTPListView::OnFilePrint)
END_MESSAGE_MAP()
...
{
	if (CXTPListView::OnCreate(lpCreateStruct) == -1)
		return -1;
...
{
	CXTPSortClass csc (&GetListCtrl(), m_nSortedCol);
...
void CGUI_OutlookView::InitializeHeaderControl()
{
	...
	for (nCol = 0; nCol < _countof(arHeaderBmps); ++nCol) {
		m_wndHeader.SetBitmap(nCol, arHeaderBmps[nCol]/*, HDF_STRING*/);
	}

	// use sort arrows.
	m_wndHeader.ShowSortArrow(TRUE);
}
...
void CGUI_OutlookView::InitializeListControl()
{
	...
	m_wndHeader.EnableAutoSize();

	// lock the first two colums from sizing operations.
	m_wndHeader.FreezeColumn(0);
	m_wndHeader.FreezeColumn(1);
}
