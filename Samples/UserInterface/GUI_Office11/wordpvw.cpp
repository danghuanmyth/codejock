...
BEGIN_MESSAGE_MAP(CWordPadView, CRichEditView)
	...
	ON_UPDATE_COMMAND_UI(ID_GALLERY_FONTTEXTCOLOR, OnUpdateSelectorTextRibbon)
	ON_XTP_EXECUTE(ID_GALLERY_FONTTEXTCOLOR, OnSelectorTextRibbon)
	...
	ON_COMMAND(ID_BACKCOLOR_NOCOLORS, OnTextAuto)
	ON_UPDATE_COMMAND_UI(ID_BACKCOLOR_NOCOLORS, OnUpdateTextAuto)
...
//	ON_COMMAND(ID_TEXT_INSERTOBJECT, CRichEditView::OnInsertObject)
	...
	ON_UPDATE_COMMAND_UI(ID_TEXT_COLOR, OnUpdateTextRibbon)
	ON_COMMAND(ID_TEXT_COLOR, OnButtonTextRibbon)

	ON_COMMAND(ID_BACKCOLOR_NOCOLOR, OnButtonBackNoColorRibbon)
	ON_UPDATE_COMMAND_UI(ID_BACKCOLOR_NOCOLOR, OnUpdateBackNoColorRibbon)
...
void CWordPadView::OnGalleryFontBackColor(NMHDR* pNMHDR, LRESULT* pResult)
{
	NMXTPCONTROL* tagNMCONTROL = (NMXTPCONTROL*)pNMHDR;
...
void CWordPadView::OnUpdateGalleryFontBackColor(CCmdUI* pCmdUI)
{
...
void CWordPadView::OnUpdateBack(CCmdUI* pCmdUI)
{
...
void CWordPadView::OnButtonBack()
{
	CHARFORMAT2 cfm;
...
void CWordPadView::OnButtonBackNoColor()
{
	CHARFORMAT2 cfm;
	cfm.cbSize =  sizeof(CHARFORMAT2);
...
void CWordPadView::OnUpdateBackNoColor(CCmdUI* pCmdUI)
{
...
//Font related message handlers

void CWordPadView::OnUpdateGalleryFontFace(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(TRUE);
}

void CWordPadView::OnUpdateGalleryFontSize(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(TRUE);
}

void CWordPadView::OnUpdateRibbonComboFont(CCmdUI* pCmd)
{
...
void CWordPadView::OnEditFont(NMHDR* pNMHDR, LRESULT* pResult)
{
	USES_CONVERSION;
...
void CWordPadView::OnUpdateRibbonComboSize(CCmdUI* pCmd)
{
	CXTPControlComboBox* pFontCombo = (CXTPControlComboBox*)CXTPControl::FromUI(pCmd);
...
void CWordPadView::OnEditSize(NMHDR* pNMHDR, LRESULT* pResult)
{
	NMXTPCONTROL* tagNMCONTROL = (NMXTPCONTROL*)pNMHDR;
...
void CWordPadView::OnUpdateTextRibbon(CCmdUI* pCmd)
{
...
void CWordPadView::OnButtonTextRibbon()
{
	CHARFORMAT& cfm = GetCharFormatSelection( );
	cfm.dwMask |= CFM_COLOR;
	cfm.dwEffects &= ~CFE_AUTOCOLOR ;
...
void CWordPadView::OnUpdateSelectorTextRibbon(CCmdUI* pCmdUI)
{
	CXTPControlGallery* pControl = 	DYNAMIC_DOWNCAST(CXTPControlGallery, CXTPControl::FromUI(pCmdUI));
...
void CWordPadView::OnSelectorTextRibbon(NMHDR* pNMHDR, LRESULT* pResult)
{
	NMXTPCONTROL* tagNMCONTROL = (NMXTPCONTROL*)pNMHDR;
...
void CWordPadView::OnButtonBackNoColorRibbon()
{
	CHARFORMAT2 cfm;
	cfm.cbSize =  sizeof(CHARFORMAT2);
...
void CWordPadView::OnUpdateBackNoColorRibbon(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(m_clrBack == COLORREF_NULL ? 1 : 0);
	pCmdUI->Enable(TRUE);
}
