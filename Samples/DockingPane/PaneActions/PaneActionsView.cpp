...
void CPaneActionsView::AddAction(XTP_DOCKINGPANE_ACTION* pAction)
{
	...
	BOOL bCancelable = (pAction->action % 2 == 0 && pAction->action < xtpPaneActionActivated)
		|| (pAction->action == xtpPaneActionDetaching) || (pAction->action == xtpPaneActionDragging)
		 || (pAction->action == xtpPaneActionUnpinning) || (pAction->action == xtpPaneActionSplitterResizing);
