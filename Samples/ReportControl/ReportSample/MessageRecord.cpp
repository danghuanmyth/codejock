					...
					CString strPreview)
{
	...
	m_pItemIcon = new CMessageRecordItemIcon(bRead);
	AddItem(m_pItemIcon);

	AddItem(new CMessageRecordItemAttachment(nAttachmentBitmap));
	
	m_pItemFrom = new CXTPReportRecordItemText(strFromName);
	AddItem(m_pItemFrom);

	m_pItemSubject = new CXTPReportRecordItemText(strSubject);
	AddItem(m_pItemSubject);

	m_pItemReceived = new CMessageRecordItemDate(odtReceived);
	AddItem(m_pItemReceived);

	m_pItemSize = new CXTPReportRecordItemNumber(nMessageSize);
	AddItem(m_pItemSize);

	CMessageRecordItemCheck *pItemCheck = new CMessageRecordItemCheck(bChecked);
#if _XTPLIB_VERSION_PREFIX >= 1511
	pItemCheck->SetCanEditCheckboxItem(TRUE);
#endif
	AddItem(pItemCheck);
...
BOOL CMessageRecord::SetRead(BOOL bRead)
{
	BOOL bUpdate = FALSE;

	if (m_pItemIcon->m_bRead != bRead)
	{
		bUpdate = TRUE;
		m_pItemIcon->m_bRead = bRead;
