...
//MD - this is just example of auto-generated strings for cell caption
	...
	if (pView && pView->m_bSimpleTree)
	{
		...
		else
		{
			...
			//pDrawArgs->pRow->SetVisible(FALSE);
			if (pDrawArgs->pColumn->GetIndex() == 0)
			{
...
//User can put here extra validation, promt to confirm or fill audit trail record about modifications
...
void CVirtualListView::OnReportRowExpandChanged(NMHDR *pNotifyStruct, LRESULT *result)
{
	...
	if ((NULL != pItemNotify) && (NULL != pItemNotify->pRow))
	{
		CXTPReportRow *pRow = pItemNotify->pRow;

		int nIndex = pRow->GetIndex();

		POSITION pos = m_lstCollapsed.Find(nIndex);
