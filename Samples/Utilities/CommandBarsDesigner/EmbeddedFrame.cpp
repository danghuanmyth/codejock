...
void (*_afxForceVectorDelete)() = &CEmbeddedFrame::ForceVectorDelete;


// We need create some controls for static version
void CEmbeddedFrame::ForceVectorDelete()
{
	ASSERT(FALSE);  // never called

	new CXTPControlColorSelector[2];
	new CXTPControlPopupColor[2];
	new CXTPControlScrollBar[2];
	new CXTPControlSlider[2];
	new CXTPControlListBox[2];
	new CXTPControlHyperlink[2];
	new CXTPControlMarkupLabel[2];
	new CXTPControlSlider[2];
	new CXTPControlProgress[2];
	new CXTPControlBitmap[2];
	new CXTPControlFontComboBox[2];
	new CXTPRibbonControlSystemPopupBarButton[2];
	new CXTPRibbonControlSystemPopupBarListItem[2];
	new CXTPRibbonControlSystemPopupBarListCaption[2];
	new CXTPRibbonControlSystemRecentFileList[2];
}
...
void CEmbeddedFrame::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->IsStoring())
	{
		for (i = 0; i < pCommandBars->GetActions()->GetCount(); i++)
		{
			CXTPControlAction* pAction = pCommandBars->GetActions()->GetAt(i);
			if (pAction->GetKey().IsEmpty())
			{
				CResourceInfo* pInfo = ResourceManager()->Find(pAction->GetID());
				if (pInfo)
				{
					pAction->SetKey(pInfo->m_strCaption);
				}
			}
		}
...
// Save Docking Position and Controls
	...
	{
		pCommandBars->SetTheme(xtpThemeRibbon); // In Case Ribbon will be loaded
...
{
	BOOL bRibbonBar = GetCommandBars()->GetMenuBar() && GetCommandBars()->GetMenuBar()->IsRibbonBar();

	m_theme = bRibbonBar ? xtpThemeRibbon : theme;
	
	EnableOffice2007Frame(NULL);

	GetCommandBars()->SetTheme(m_theme);

	if (bRibbonBar)
	{
		 ((CXTPRibbonBar*)GetCommandBars()->GetMenuBar())->EnableFrameTheme();
	}
	else
	{
		EnableOffice2007Frame(theme == xtpThemeResource || theme == xtpThemeRibbon ? GetCommandBars() : NULL);
	}
}
