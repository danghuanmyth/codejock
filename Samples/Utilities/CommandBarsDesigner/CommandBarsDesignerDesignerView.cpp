...
void CCommandBarsDesignerDesignerView::OnEditCut()
{
	CXTPCommandBars* pCommandBars = m_pFrame->GetCommandBars();
	if (!pCommandBars)
		return;

	CCmdTarget* pDragContrl = pCommandBars->GetDragControl();
	if (!pDragContrl)
		return;
	
	CXTPControl* pControl = DYNAMIC_DOWNCAST(CXTPControl, pDragContrl);
	...
	CXTPRibbonGroup* pRibbonGroup = DYNAMIC_DOWNCAST(CXTPRibbonGroup, pDragContrl);
	
	if (pRibbonGroup)
	{
		pRibbonGroup->GetGroups()->Remove(pRibbonGroup);
		
		pCommandBars->SetDragControl(NULL);
		return;
	}
	
	CXTPRibbonTab* pRibbonTab = DYNAMIC_DOWNCAST(CXTPRibbonTab, pDragContrl);
...
void CCommandBarsDesignerDesignerView::OnUpdateEditCut(CCmdUI* pCmdUI)
{
	CXTPCommandBars* pCommandBars = m_pFrame->GetCommandBars();
	if (!pCommandBars)
		return;
	...
	if (pControl)
	{
		if (pControl->GetFlags() & xtpFlagNoMovable)
		{
			pCmdUI->Enable(FALSE);
			return;
		}
