...
CString GetStringID(CResourceManager* pResourceManager, int nID)
{
	if (pResourceManager)
		return pResourceManager->GetStringID(nID);
...
CXTPImageManagerIconHandle& CImage::GetHandle()
{
	...
	state == xtpImageChecked? pIcon->GetCheckedIcon(): state == xtpImagePressed? pIcon->GetPressedIcon(): pIcon->GetDisabledIcon();
}
...
void CDialogBitmapExport::OnUpdateIcons()
{
	...
	while (pos)
	{
		...
		if (bFound)
		{
			CString strResource = GetStringID(m_pResourceManager, nID);
...
void CDialogBitmapExport::OnImageChanged(CImage* pImage)
{
	...
	m_comboState.AddString(_T("xtpImagePressed"));
	...
	if (pImage != NULL)
	{
		CString strResource = GetStringID(m_pResourceManager, pImage->nID);
