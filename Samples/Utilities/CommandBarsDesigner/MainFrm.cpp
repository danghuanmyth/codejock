...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CXTPDockingPane* pPaneLibrary = m_paneManager.CreatePane(
		IDR_PANE_LIBRARY, CRect(0, 0,200, 120), xtpPaneDockBottom, pPaneIcons);
	...
	if (wParam == XTP_DPN_SHOWWINDOW)
	{
		...
		if (!pPane->IsValid())
		{
			switch (pPane->GetID())
			{
				...
				case IDR_PANE_LIBRARY:
				{
					if (m_paneLibrary.GetSafeHwnd() == 0)
					{
						m_paneLibrary.Create(_T("STATIC"), NULL, WS_CHILD|WS_CLIPCHILDREN|WS_CLIPSIBLINGS, CRect(0, 0, 0, 0), this, 0);
					}

					pPane->Attach(&m_paneLibrary);
					break;
				}
			}
			RefreshPanes();
...
void CMainFrame::RefreshPanes(BOOL bForceAll)
{
	...
	if (m_paneProperties.GetSafeHwnd()) m_paneProperties.Refresh(m_pActivePane);
...
{
	m_paneManager.ShowPane(IDR_PANE_LIBRARY);
}
