...
void CPaneControls::OnPanecontrolsAdd()
{
	...
	if (dc.DoModal() == IDOK)
	{
		XTPControlType type = xtpControlError;
		...
		if (type == xtpControlError && !dc.m_strClass.IsEmpty())
		{
			CRuntimeClass* pClass = CXTPPropExchange::FindRuntimeClass(T2A((LPTSTR)(LPCTSTR)dc.m_strClass));
			if (pClass && pClass->IsDerivedFrom(RUNTIME_CLASS(CXTPControl)))
			{
				pControl = (CXTPControl*)pClass->CreateObject();
				if (pControl) pFrame->m_pControls->Add(pControl, nID);
			}

		}

		if (type == xtpControlError && !pControl)
			type = xtpControlButton;

		if (!pControl)
		{
			pControl = pFrame->m_pControls->Add(type, nID);
		}
