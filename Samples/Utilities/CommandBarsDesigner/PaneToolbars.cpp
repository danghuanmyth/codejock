...
BEGIN_MESSAGE_MAP(CPaneToolbars, CWnd)
	...
	ON_COMMAND(ID_PANETOOLBARS_ADDRIBBONBAR, OnPanetoolbarsAddRibbon)
	ON_UPDATE_COMMAND_UI(ID_PANETOOLBARS_ADDRIBBONBAR, OnUpdatePanetoolbarsAddRibbon)
...
int CPaneToolbars::BuildToolbarsTree(HTREEITEM hItem, CXTPCommandBar* pCommandBar, BOOL bCheck)
{
	...
	for (int i = 0; i < pCommandBar->GetControlCount(); i++)
	{
		...
		if ((pControl == NULL) || (pPopupBar == NULL) || (pControl->GetID() == XTP_ID_TOOLBAR_EXPAND) 
			|| (pControl->GetID() == XTP_ID_RIBBONCONTROLQUICKACCESS) || pControl->IsKindOf(RUNTIME_CLASS(CXTPRibbonGroupControlPopup)))
			continue;
...
int CPaneToolbars::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndToolBar.GetControl(2)->SetVisible(FALSE);

	CXTPControlPopup* pControl = (CXTPControlPopup*)m_wndToolBar.GetControls()->SetControlType(0, xtpControlPopup);
	pControl->SetStyle(xtpButtonIconAndCaption);
	CMenu menu;
	menu.LoadMenu(ID_PANETOOLBARS_ADD);
	pControl->SetCommandBar(menu.GetSubMenu(0));
...
CXTPRibbonBar* CPaneToolbars::GetRibbonBar()
{
	if (m_pActiveCommandBars && m_pActiveCommandBars->GetMenuBar() && m_pActiveCommandBars->GetMenuBar()->IsRibbonBar())
	{
		return (CXTPRibbonBar*)m_pActiveCommandBars->GetMenuBar();
	}

	return NULL;
}

void CPaneToolbars::RefreshVerbs()
{
	...
	if (pRibbonBar)
	{
		if (pRibbonBar->GetSystemButton() == 0)
		{
			m_wndPropertyGrid.GetVerbs()->Add(_T("Add Ribbon System Button"), 1);
		}

		m_wndPropertyGrid.GetVerbs()->Add(_T("Add Ribbon Tab"), 2);

		if (pRibbonBar->GetSelectedTab() != NULL)
		{
			m_wndPropertyGrid.GetVerbs()->Add(_T("Add Ribbon Group"), 3);			
		}
	}
...
void CPaneToolbars::OnPanetoolbarsAddRibbon()
{
	if (m_pActiveCommandBars == NULL)
		return;

	if (m_pActiveCommandBars->GetMenuBar() != NULL && m_pActiveCommandBars->GetMenuBar()->IsRibbonBar())
		return;
	...
	CXTPMenuBar* pMenuBar = m_pActiveCommandBars->GetMenuBar();

	m_pActiveCommandBars->SetTheme(xtpThemeRibbon);
	CXTPRibbonBar* pRibbonBar = (CXTPRibbonBar*)m_pActiveCommandBars->Add(_T("The Ribbon"), xtpBarTop, RUNTIME_CLASS(CXTPRibbonBar));

	CXTPDockBar* pDockBar = m_pActiveCommandBars->GetDockBar(xtpBarTop);
	m_pActiveCommandBars->DockCommandBar(pRibbonBar, CRect(0, 0, 10, 10), pDockBar);


	if (pMenuBar)
	{
		for (int i = 0; i < pMenuBar->GetControlCount(); i++)
		{
			CXTPControl* pControl = pMenuBar->GetControl(i);
			pRibbonBar->GetControls()->AddClone(pControl);
		}
		m_pActiveCommandBars->Remove(pMenuBar);
	}


	pRibbonBar->EnableCustomization();

	pFrame->SetTheme(xtpThemeRibbon);

	Refresh();
	
	HTREEITEM hItem = (HTREEITEM)pRibbonBar->GetCommandBarData();
	m_wndTreeCtrl.SelectItem(hItem);

	RefreshVerbs();
}

void CPaneToolbars::OnUpdatePanetoolbarsAddRibbon(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(m_pActiveCommandBars != NULL && 
		(m_pActiveCommandBars->GetMenuBar() == NULL || !m_pActiveCommandBars->GetMenuBar()->IsRibbonBar()));
}
...
LRESULT CPaneToolbars::OnGridNotify(WPARAM wParam, LPARAM lParam)
{
	if (wParam == XTP_PGN_VERB_CLICK)
	{
		...
		if (pVerb->GetID() == 1)
		{
			...
			pControlFile->GetCommandBar()->SetTitle(_T("System Menu"));
			
			UINT uCommand = {ID_MENU_FILE};
			pRibbonBar->GetImageManager()->SetIcons(ID_MENU_FILE, &uCommand, 1, CSize(0, 0), xtpImageNormal);
			
			pFrame->ResourceManager()->Set(ID_MENU_FILE, _T("ID_MENU_FILE"));
		}
		
		if (pVerb->GetID() == 2)
		{
			CXTPRibbonTab* pTab = pRibbonBar->AddTab(_T("New Tab"));
			pTab->Select();
		...
		if (pVerb->GetID() == 3)
		{
			CXTPRibbonTab* pTab = pRibbonBar->GetSelectedTab();

			CXTPRibbonGroup* pGroup = pTab->GetGroups()->Add(_T("New Group"));

			pRibbonBar->GetCommandBars()->SetDragControl(pGroup);
		}
