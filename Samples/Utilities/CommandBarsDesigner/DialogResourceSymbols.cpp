...
void CDialogResourceSymbols::OnButtonExport()
{
	...
	if (fd.DoModal() == IDOK)
	{
		CStdioFile file(fd.GetPathName(), CFile::modeCreate|CFile::modeWrite);

		if (fd.GetPathName().Right(2).CompareNoCase(_T(".h"))== 0)
		{
...
void CDialogResourceSymbols::OnButtonDelete()
{
	...
	if (nSel != LB_ERR)
	{
		...
		m_pFrame->GetCommandBars()->GetActions()->Remove(pInfo->m_nID);
...
void CDialogResourceSymbols::OnButtonRename()
{
	int nSel = m_wndList.GetCurSel();
	if (nSel != LB_ERR)
	{
		...
		CDialogRenameSymbol ns;

		ns.m_strName = pInfo->m_strCaption;
		ns.m_nValue = pInfo->m_nID;
		
		if (ns.DoModal() == IDOK && ns.m_strName != pInfo->m_strCaption)
		{
			
			CResourceInfo* pInfo;
			CString strCaption;
			
			if (!pMapResources->Lookup(ns.m_strName, (CObject*&)pInfo))
			{
				m_pFrame->ResourceManager()->Set(ns.m_nValue, ns.m_strName);
				
				m_wndList.Invalidate();

				CXTPControlAction* pAction = m_pFrame->GetCommandBars()->GetActions()->FindAction(ns.m_nValue);
				if (pAction)
				{
					pAction->SetKey(ns.m_strName);
				}
			}
		}
	}
...
void CDialogResourceSymbols::OnSelchangeList()
{
	...
	GetDlgItem(IDC_BUTTON_RENAME)->EnableWindow(nSel != LB_ERR);
