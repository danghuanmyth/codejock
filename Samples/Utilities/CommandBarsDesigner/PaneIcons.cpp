...
void CPaneIcons::ShowIcons(CXTPImageManager* pImageManager, int nActive, CResourceManager* pResourceManager)
{
	if (!m_hWnd)
		return;
	...
	m_pActiveResourceManager = pResourceManager;
...
void CPaneIcons::Refresh(BOOL bForce)
{
	...
	if (m_pActiveFrame)
	{
		CXTPControl* pControl = DYNAMIC_DOWNCAST(CXTPControl, m_pActiveFrame->GetCommandBars()->GetDragControl());
...
void CPaneIcons::OnPaneiconsNew()
{
	...
	CDialogNewIcon dni;
	dni.m_pResourceManager = m_pActiveResourceManager;
