...
void CGrepView::OnReportItemDblClick(NMHDR * pNotifyStruct, LRESULT * /*result*/)
{
	...
	if (pItemNotify->pRow)
	{
		...
		if (pRecord)
		{
			if (g_settings.m_bUseEditor)
				ShellExecute(0, 0, g_settings.m_strEditor, (LPTSTR)(LPCTSTR)pRecord->m_strPath, 0, SW_SHOW);
			else
				ShellExecute(0, _T("open"), (LPTSTR)(LPCTSTR)pRecord->m_strPath, 0, 0, SW_SHOW);
		}
	}
}
