...
//	ON_COMMAND(ID_CALENDAR_LANGUAGE_DEFAULT,  OnCalendarLangDefault)
	...
	ON_COMMAND(ID_CALENDAR_TIMELINE, OnCalendarTimeline)
	ON_UPDATE_COMMAND_UI(ID_CALENDAR_TIMELINE, OnUpdateCalendarTimeline)

	ON_COMMAND(ID_VIEW_JUSTFORTEST, OnJust4Test)
END_MESSAGE_MAP()
...
//	m_Sink.Advise(ptrCalendarConn, XTP_NC_CALENDAR_ON_REMINDERS, &CCalendarDemoView::OnReminders);
	...
	if (pDayView)
	{
		COleDateTime dtMinTime;
		dtMinTime.SetTime(6, 0, 0);

		COleDateTime dtMaxTime;
		dtMaxTime.SetTime(23, 59, 59);

		pDayView->SetScaleMinTime(dtMinTime);
		pDayView->SetScaleMaxTime(dtMaxTime);

		GetCalendarCtrl().GetCalendarOptions()->dtScaleMinTime = dtMinTime;
		GetCalendarCtrl().GetCalendarOptions()->dtScaleMaxTime = dtMaxTime;

	}


	GetCalendarCtrl().m_bTimelineMode = FALSE;
}
...
void CCalendarDemoView::SetView(XTPCalendarViewType viewType)
{
	...
	//{
	//	GetCalendarCtrl().GetActiveView()->SetDayHeaderFormatLong(_T("MMMM d"));
	//	GetCalendarCtrl().GetActiveView()->SetDayHeaderFormatShort(_T("d"));
	//}
...
{
	GetCalendarCtrl().m_nTimelineScale = xtpTSPID_Day;
	...
	{
		SwitchToTimeLineView(xtpTSPID_Day);	
	}
}

void CCalendarDemoView::OnCalendarViewWorkWeek() 
{
	GetCalendarCtrl().m_nTimelineScale = xtpTSPID_Week;
	if (!GetCalendarCtrl().m_bTimelineMode)
	{
		if (GetCalendarCtrl().GetActiveView() && 
	...
	{
		SwitchToTimeLineView(xtpTSPID_WorkWeek);
	}
}
...
void CCalendarDemoView::OnCalendarViewWeek() 
{
	...
	{
		SwitchToTimeLineView(xtpTSPID_Week);
	}
}

void CCalendarDemoView::OnCalendarViewMonth() 
{   
	GetCalendarCtrl().m_nTimelineScale = xtpTSPID_Month;
	if (!GetCalendarCtrl().m_bTimelineMode)
	{
		if (GetCalendarCtrl().GetActiveView() && 
	...
	{
		SwitchToTimeLineView(xtpTSPID_Month);
	}
...
void CCalendarDemoView::SwitchToTimeLineView(int eScaleID)
{		
	GetCalendarCtrl().m_bTimelineMode = TRUE;
	GetCalendarCtrl().m_nTimelineScale = eScaleID;

	SetView(xtpCalendarTimeLineView);

	CXTPCalendarTimeLineView* pTLView = DYNAMIC_DOWNCAST(CXTPCalendarTimeLineView, GetCalendarCtrl().GetActiveView());
	if (pTLView)
	{
		COleDateTime dtNow(COleDateTime::GetCurrentTime());
		pTLView->SetStartViewDate(dtNow);
		pTLView->SetTimeScaleID(eScaleID);
	}
}

BOOL CCalendarDemoView::IsTimeLineViewWithScale(int eScaleID)
{
	BOOL bTLV_result = FALSE;
	
	BOOL bTLView = (GetCalendarCtrl().GetActiveView()->GetViewType() == xtpCalendarTimeLineView);	
	if (bTLView) 
	{
		CXTPCalendarTimeLineView* pTLView = DYNAMIC_DOWNCAST(CXTPCalendarTimeLineView, GetCalendarCtrl().GetActiveView());
		bTLV_result = pTLView->GetTimeScaleID() == eScaleID;
	}
	return bTLV_result;	
}
...
void CCalendarDemoView::OnUpdateCalendarViewDay(CCmdUI* pCmdUI) 
{
	...
	if (pView && (pView->GetViewType() == xtpCalendarDayView ||
		IsTimeLineViewWithScale(xtpTSPID_Day)))
		bViewDay = TRUE;			
...
void CCalendarDemoView::OnUpdateCalendarViewWorkWeek(CCmdUI* pCmdUI) 
{
	...
	if (pView && (pView->GetViewType() == xtpCalendarWorkWeekView ||
		IsTimeLineViewWithScale(xtpTSPID_Week)))
		bViewDays = TRUE;			
...
void CCalendarDemoView::OnUpdateCalendarViewMonth(CCmdUI* pCmdUI) 
{
	...
	if (pView && (pView->GetViewType() == xtpCalendarMonthView ||
		IsTimeLineViewWithScale(xtpTSPID_Month))) 
		bViewMonth = TRUE; 
...
//	
//	((CMainFrame*) GetParentFrame())->m_dlgDatePicker.m_wndDatePicker.AdjustLayout();
//	((CMainFrame*) GetParentFrame())->m_dlgDatePicker.m_wndDatePicker.RedrawControl();
//}
//
...
void CCalendarDemoView::AddNewEvent(CPoint* /*pPtHitContext*/) 
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		DWORD dNewEventID = dlg.GetLastEventID();
		if (dNewEventID > 0)
		{
			CXTPCalendarEventPtr evPtr = pCalendarData->GetEvent(dNewEventID);
			CXTPCalendarView* pView = GetCalendarCtrl().GetActiveView();
			if (pView && (pView->GetViewType() == xtpCalendarMonthView))
			{
				for (UINT i = 0; i < pView->GetDiscreteSelectionCount(); i++)
				{
					COleDateTime dt = pView->GetDiscreteSelectionValue(i);
					if (dt.m_status == COleDateTime::valid)
					{
						//AfxMessageBox(dt.Format());
						//Need a function to copy event with new datetime
					}
				}
			}
		}
...
void CCalendarDemoView::AddNewTimeEvent(XTP_CALENDAR_HITTESTINFO hitInfo)
{
	COleDateTime dtInitial(hitInfo.dt);
	CXTPCalendarEventPropertiesDlg dlg(&GetCalendarCtrl(), 
		TRUE, this, NULL, XTP_IDD_CALENDAR_EVENT_PROPERTIES, dtInitial);
	if (dlg.DoModal() == IDOK)
	{
		CXTPCalendarData* pCalendarData = GetCalendarCtrl().GetDataProvider();
		DWORD dNewEventID = dlg.GetLastEventID();
...
void CCalendarDemoView::OnInitialUpdate() 
{
	...
	//pWnd->m_dlgDatePicker.m_wndDatePicker.AllowNoncontinuousSelection(FALSE);//default - TRUE
	//---------------------------------------------------------------------------
	...
	if (pDayView) 
	{
		// test min and max scale times
		//pDayView->SetScaleMinTime(0.25);
		//pDayView->SetScaleMaxTime(0.75);

		// initialize
		//pDayView->m_bDraftMode = TRUE;
		//pDayView->m_bEcoMode = TRUE;
		pDayView->ScrollToWorkDayBegin();
	}
	//GetCalendarCtrl().GetCalendarOptions()->nDayView_ScaleInterval = 10; //5;
	...
	GetCalendarCaptionBarCtrl().EnableTimeline(TRUE);

	ShowCaptionBar(TRUE);

	GetCalendarCtrl().GetDayView()->SetDayHeaderFormatLong(_T("MMMM d"));
	GetCalendarCtrl().GetWeekView()->SetDayHeaderFormatLong(_T("MMMM d"));
	GetCalendarCtrl().GetMonthView()->SetDayHeaderFormatLong(_T("MMMM d"));
	GetCalendarCtrl().SetLongDayHeaderFormat(_T("MMMM d"));
	GetCalendarCtrl().SetSmallDayHeaderFormat(_T("d")); 


	OnOfficeThemeChanged(xtpCalendarThemeResource);
	OnCalendarViewMonth();


	//GetCalendarCtrl().m_bMultiColumnWeekMode = FALSE;
	COleDateTime dt;
	dt.SetTime(0,0,0);
	GetPrintOptions()->m_dtPrintFrom = dt;
	dt.SetTime(23,59,59);
	GetPrintOptions()->m_dtPrintTo = dt;
	...
	else if (XTP_NC_CALENDARCONTEXTMENU == Event)
	{
		if (bHitTest)
		{
				...
				switch (nMenuResult)
				{
					case ID_POPUP_COPYEVENT:
					{
						...
						if (hitInfo.pViewEvent)
						{
							...
							if (pEvent)
							{
								GetCalendarCtrl().GetActiveView()->SelectEvent(pEvent);
								GetCalendarCtrl().OnCopy();
							}
						}
					}
					break;
			...
			else //if (GetCalendarCtrl().GetActiveView()->IsKindOf(RUNTIME_CLASS(CXTPCalendarWeekView)))
			{
				...
				if (GetCalendarCtrl().GetActiveView()->IsKindOf(RUNTIME_CLASS(CXTPCalendarTimeLineView)))
					menu.GetSubMenu(0)->RemoveMenu(ID_POPUP_GOTOTHISDAY, MF_BYCOMMAND);
...
void CCalendarDemoView::OnOfficeThemeChanged(int nCalendarTheme)
{
	...
	if (nCalendarTheme == xtpCalendarThemeResource)
	{
		CXTPCalendarThemeOffice2007* pTheme = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007, GetCalendarCtrl().GetTheme());
		if (pTheme)
		{
			CXTPImageManager* pIM = pTheme->GetCustomIconsList();
			if (pIM)
			{
				pIM->SetIcon(IDI_MEETING_ICON, (UINT)xtpCalendarEventIconIDMeeting, CSize(16, 16));
				pIM->SetIcon(IDI_PRIVATE_ICON, (UINT)xtpCalendarEventIconIDPrivate, CSize(16, 16));
			}
		}
	}
...
void CCalendarDemoView::OnCalendarTimeline()
{
	GetCalendarCtrl().OnCalendarTimeline();
}

void CCalendarDemoView::OnUpdateCalendarTimeline(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(GetCalendarCtrl().m_bTimelineMode);
}


void CCalendarDemoView::OnJust4Test()
{
}

void CCalendarDemoView::OnFilePrintPreview()
{
