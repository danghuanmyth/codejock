...
void CAdvancedCalendarOptionsDlg::OnOK() 
{
	...
	if (nEnableThemes)
	{
		if (m_ctrlThemeOffice2007.GetCheck())
		{
			...
			pTheme2007->GetCustomIconsList()->SetIcon(IDI_MEETING_ICON, 
				(UINT)xtpCalendarEventIconIDMeeting, CSize(16, 16));

			pTheme2007->GetCustomIconsList()->SetIcon(IDI_PRIVATE_ICON, 
				(UINT)xtpCalendarEventIconIDPrivate, CSize(16, 16));
