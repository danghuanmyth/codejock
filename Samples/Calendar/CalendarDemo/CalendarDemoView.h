...
protected:
	void SwitchToTimeLineView(int eScaleID);
	BOOL IsTimeLineViewWithScale(int eScaleID);
...
//	afx_msg void OnCalendarLangDefault();
	...
	void OnEvent_GetItemText(XTP_NOTIFY_CODE Event, WPARAM wParam , LPARAM lParam);
...
public:
	...
	void AddNewTimeEvent(XTP_CALENDAR_HITTESTINFO hitInfo);

	afx_msg void OnBnClickedScrollDayPrev();
	afx_msg void OnBnClickedIdcScrollDayNext();
//MD	
	afx_msg void OnCalendarTimeline();
	afx_msg void OnUpdateCalendarTimeline(CCmdUI *pCmdUI);
//MD
	...
	int m_nScale;
//MD - moved to CXTPCalendarControl class
	//BOOL m_bTimelineMode;
	//XTPCalendarViewType m_eViewType;
	//int m_nTimelineScale;
//MD
	afx_msg void OnJust4Test();
};
