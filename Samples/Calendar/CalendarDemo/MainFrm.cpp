...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	SetOfficeTheme(xtpCalendarThemeResource);
...
BOOL IsStyleExists(CString strPath)
{
	int nIndex = strPath.Find(',');
	if (nIndex != -1)
		strPath = strPath.Left(nIndex);

	return FILEEXISTS_S(strPath);
}
...
int CMainFrame::OnCreateControl(LPCREATECONTROLSTRUCT lpCreateControl)
{
	if (lpCreateControl->nID == ID_VIEW_OFFICE_THEME)
	{
		...
		pComboTheme->SetItemData(nIndexBlue, xtpCalendarThemeResource);

		// 0 - Blue, 1 - Silver, 2 - Black
		// 0 - Blue, 1 - Silver, 2 - Black, 3 - Aqua, 4 - Scenic, 5 - White
		if (IsStyleExists(_GetTheme2007Style_file(1)))
		{
			nIndex = pComboTheme->AddString(_T("Office 2007 (Silver)"));
			pComboTheme->SetItemData(nIndex, xtpCalendarThemeResource + 100 + 1);
		}

		if (IsStyleExists(_GetTheme2007Style_file(2)))
		{
			nIndex = pComboTheme->AddString(_T("Office 2007 (Black)"));
			pComboTheme->SetItemData(nIndex, xtpCalendarThemeResource + 100 + 2);
		}
			
		if (IsStyleExists(_GetTheme2007Style_file(3)))
		{
			nIndex = pComboTheme->AddString(_T("Office 2007 (Aqua)"));
			pComboTheme->SetItemData(nIndex, xtpCalendarThemeResource + 100 + 3);
		}

		if (IsStyleExists(_GetTheme2007Style_file(4)))
		{
			nIndex = pComboTheme->AddString(_T("Windows 7 (Scenic)"));
			pComboTheme->SetItemData(nIndex, xtpCalendarThemeResource + 100 + 4);
		}

		if (IsStyleExists(_GetTheme2007Style_file(6)))
		{
			nIndex = pComboTheme->AddString(_T("Office 2010 (Blue)"));
			pComboTheme->SetItemData(nIndex, xtpCalendarThemeResource + 100 + 6);
		}

		if (IsStyleExists(_GetTheme2007Style_file(5)))
		{
			nIndex = pComboTheme->AddString(_T("Office 2010 (Silver)"));
			pComboTheme->SetItemData(nIndex, xtpCalendarThemeResource + 100 + 5);
		}

		if (IsStyleExists(_GetTheme2007Style_file(7)))
		{
			nIndex = pComboTheme->AddString(_T("Office 2010 (Black)"));
			pComboTheme->SetItemData(nIndex, xtpCalendarThemeResource + 100 + 7);
		}
...
void CMainFrame::OnOfficeThemeChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if ((GetKeyState(VK_SHIFT) & 0x8000) && (GetKeyState(VK_CONTROL) & 0x8000))
	{
		if (IsStyleExists(_GetTheme2007Style_file(3)))
		{
			...
			pControl->SetItemData(nIndex, xtpCalendarThemeResource + 100 + 3);
			pControl->SetCurSel(nIndex);

			nTheme = xtpCalendarThemeResource + 100 + 3;
		}
	}
#endif
...
//	xtpThemeCustom          // Custom theme.
	...
	{
		nTheme2007Style = nCalendarTheme - xtpCalendarThemeResource - 100;
		nCalendarTheme = xtpCalendarThemeResource;
	}	

	//------------------------------------
	CString strFilePath = _GetTheme2007Style_file(nTheme2007Style);
	XTPResourceImages()->SetHandle(strFilePath);
	//===============================================================
	...
	if (sarThemesMap[xtpCalendarThemeOffice2000] == 0)
	{
		...
		sarThemesMap[xtpCalendarThemeResource] = xtpThemeRibbon;
	}
...
// nTheme2007Style: 0 - Blue, 1 - Silver, 2 - Black, 3 - Aqua, 4 - Scenic, 5 - White
CString CMainFrame::_GetTheme2007Style_file(int nTheme2007Style)
{
	...
	{	
		...
		{
			strFilePath += _T("Styles\\Office2007.dll,Office2007Silver.ini");	
		}
		else if (nTheme2007Style == 2)
		{
			strFilePath += _T("Styles\\Office2007.dll,Office2007Black.ini");	
		}
		else if (nTheme2007Style == 3)
		{
			strFilePath += _T("Styles\\Office2007.dll,Office2007Aqua.ini");	
		}
		else if (nTheme2007Style == 4)
		{
			strFilePath += _T("Styles\\Windows7.dll,Windows7Blue.ini");	
		}
		else if (nTheme2007Style == 5)
		{
			strFilePath += _T("Styles\\Office2010.dll,Office2010Silver.ini");	
		}
		else if (nTheme2007Style == 6)
		{
			strFilePath += _T("Styles\\Office2010.dll,Office2010Blue.ini");	
		}
		else if (nTheme2007Style == 7)
		{
			strFilePath += _T("Styles\\Office2010.dll,Office2010Black.ini");	
		}
		else 
		{
