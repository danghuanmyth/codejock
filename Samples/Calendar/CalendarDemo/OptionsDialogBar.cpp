...
BEGIN_MESSAGE_MAP(COptionsDialogBar, CDialogBar)
    ...
    ON_BN_CLICKED(IDC_CHECK_AUTO_RESET_BUSY_FLAG, OnBnClickedCheckAutoResetBusyFlag)
END_MESSAGE_MAP()
...
//<<>>
    ...
    //------------------------------------------------------------------------
    CButton* cboxAutoResetBusyFlag = (CButton*) GetDlgItem(IDC_CHECK_AUTO_RESET_BUSY_FLAG);
    cboxAutoResetBusyFlag->SetCheck(
        GetCalendarCtrl()->GetCalendarOptions()->bDayView_AutoResetBusyFlag 
        ? BST_CHECKED
        : BST_UNCHECKED);
...
void COptionsDialogBar::OnBnClickedCheckAutoResetBusyFlag()
{
    BOOL checked = (BST_CHECKED == ((CButton*) GetDlgItem(IDC_CHECK_AUTO_RESET_BUSY_FLAG))->GetCheck());
    CXTPCalendarOptions* options = const_cast<CXTPCalendarOptions*>(GetCalendarCtrl()->GetCalendarOptions());
    options->bDayView_AutoResetBusyFlag = checked;
