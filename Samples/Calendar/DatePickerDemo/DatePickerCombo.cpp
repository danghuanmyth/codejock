	...
	if (wndDatePicker.GoModal(rc, this))
	{
		...
		if (wndDatePicker.GetSelRange(dtFrom, dtTo))
		{
			if (dtFrom == dtTo)
			{
				...
				strPopup = CTime(dtFrom.GetYear(), dtFrom.GetMonth(),
					dtFrom.GetDay(), 0, 0, 0).Format(_T("%x"));
			...
			} else
			{
				...
				// it in the following way
				strPopup.Format(_T("%s - %s"),
					CTime(dtFrom.GetYear(), dtFrom.GetMonth(),
					dtFrom.GetDay(), 0, 0, 0).Format(_T("%x")), CTime(dtTo.GetYear(), dtTo.GetMonth(), dtTo.GetDay(),
					0, 0, 0).Format(_T("%x")));
