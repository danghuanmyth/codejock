...
BEGIN_MESSAGE_MAP(CDatePickerDemoDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_RADIO_CLASSIC, OnRadioClassic)
	ON_BN_CLICKED(IDC_RADIO_WINDOWSXP, OnRadioWindowsxp)
	...
	ON_BN_CLICKED(IDC_RADIO_OFFICE2007, OnRadioOffice2007)
	//}}AFX_MSG_MAP

	ON_CBN_DROPDOWN(IDC_BTN_POPUP, OnBnClickedBtnPopup)
END_MESSAGE_MAP()
...
BOOL CDatePickerDemoDlg::OnInitDialog()
{
	...
	m_ToolTip.AddTool(GetDlgItem(IDC_RADIO_WINDOWSXP),		LoadResourceString(IDS_HINT_LUNA_THEME    ));
	...
	{
		GetDlgItem(IDC_RADIO_WINDOWSXP)->EnableWindow(FALSE);
	}
	...
	m_wndDropDownButton.SubclassDlgItem(IDC_BTN_POPUP, this);
	m_wndDropDownButton.SetWindowText(_T(""));
	m_wndDropDownButton.SetPushButtonStyle(xtpButtonDropDown);

	m_wndDropDownBuddy.SubclassDlgItem(IDC_EDIT_POPUP, this);
	m_wndDropDownButton.SetBuddy(&m_wndDropDownBuddy, FALSE);
...
void CDatePickerDemoDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	if (::IsWindow(m_wndDatePicker.GetSafeHwnd()))
	{
		...
		if (!rcDP.IsRectEmpty())
		{
			...
			rcDP.right = cx - gap;
			if (!rcDP.IsRectEmpty())
				m_wndDatePicker.MoveWindow(rcDP, FALSE);

			CRect rcLeftInvalidate(rcDP.right, rcDP.top, cx, cy);
			InvalidateRect(rcLeftInvalidate, TRUE);
			
			CRect rcBottomInvalidate(rcDP.left, rcDP.bottom, cx, cy);
			InvalidateRect(rcBottomInvalidate, TRUE);
		}
...
void CDatePickerDemoDlg::OnRadioClassic() 
{
	UpdateData();
	
	m_wndDatePicker.SetTheme(NULL);
	m_wndDatePicker.GetPaintManager()->EnableTheme(FALSE);
}

void CDatePickerDemoDlg::OnRadioWindowsxp() 
{
	UpdateData();

	m_wndDatePicker.SetTheme(NULL);
	m_wndDatePicker.GetPaintManager()->EnableTheme(TRUE);
	
}

void CDatePickerDemoDlg::OnRadioOffice2007() 
{
	UpdateData();
