...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_dlgDatePicker.Create(this, IDD_DATEPICKER,
		CBRS_LEFT|CBRS_TOOLTIPS|CBRS_FLYBY, ID_VIEW_DATEPICKER))
		return -1;		// fail to create

	// no docking for DatePicker
	m_dlgDatePicker.EnableDocking(0L);
	m_dlgDatePicker.SetWindowText(_T("Date Picker"));
	ShowControlBar(&m_dlgDatePicker, FALSE, FALSE);
	FloatControlBar(&m_dlgDatePicker, CPoint(GetSystemMetrics(SM_CXSCREEN) / 3, GetSystemMetrics(SM_CYSCREEN) / 3));
...
BEGIN_MESSAGE_MAP(CDatePickerDlg, CDialogBar)
	ON_MESSAGE_VOID(WM_INITDIALOG, OnInitDialog)
END_MESSAGE_MAP()

void CDatePickerDlg::OnInitDialog() 
{
	Default();

	m_wndDatePicker.SubclassDlgItem(IDC_DATEPICKER, this);		
	m_wndDatePicker.SetShowWeekNumbers(TRUE);
	m_wndDatePicker.SetBorderStyle(xtpDatePickerBorderNone);

	CXTPClientRect rcPicker(this);
	m_wndDatePicker.MoveWindow(rcPicker);
}