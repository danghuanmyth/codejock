...
CTrayIconDlgDlg::CTrayIconDlgDlg(CWnd* pParentWnd /*=NULL*/)
	: CXTPResizeDialog(CTrayIconDlgDlg::IDD, pParentWnd)
{
...
BEGIN_MESSAGE_MAP(CTrayIconDlgDlg, CXTPResizeDialog)
...
BOOL CTrayIconDlgDlg::OnInitDialog()
{
	...
	SetResize(IDC_EDIT_TOOLTIP,      XTP_ANCHOR_TOPLEFT,     XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_BTN_TOOLTIP,       XTP_ANCHOR_TOPRIGHT,    XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_BTN_MINIMIZETRAY,  XTP_ANCHOR_TOPRIGHT,    XTP_ANCHOR_TOPRIGHT);
	SetResize(IDOK,                  XTP_ANCHOR_TOPRIGHT,    XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_TEXT_INFO,         XTP_ANCHOR_TOPLEFT,     XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_GBOX_FEATURES,     XTP_ANCHOR_TOPLEFT,     XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_GBOX_BALLOONTIP,   XTP_ANCHOR_TOPLEFT,     XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_EDIT_BALLOONTITLE, XTP_ANCHOR_TOPLEFT,     XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_TXT_TIMEOUT,       XTP_ANCHOR_TOPRIGHT,    XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_EDIT_TIMEOUT,      XTP_ANCHOR_TOPRIGHT,    XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_EDIT_BALLOONMSG,   XTP_ANCHOR_TOPLEFT,     XTP_ANCHOR_BOTTOMRIGHT);
	SetResize(IDC_TXT_BALLOONICON,   XTP_ANCHOR_BOTTOMLEFT,  XTP_ANCHOR_BOTTOMLEFT);
	SetResize(IDC_COMBO_BALLOONICON, XTP_ANCHOR_BOTTOMLEFT,  XTP_ANCHOR_BOTTOMRIGHT);
	SetResize(IDC_BTN_SHOWBALLOON,   XTP_ANCHOR_BOTTOMRIGHT, XTP_ANCHOR_BOTTOMRIGHT);

	// Load window placement
	LoadPlacement(_T("CTrayIconDlgDlg"));

	m_txtBalloonTip.SetFont(&XTPAuxData().fontBold);
	EnableControls();
...
{
	bool bEnable = XTPSystemVersion()->IsWin2KOrGreater();
