...
void CAnimateStatic::OnPaint() 
{
	...
	CXTPBufferDC memDC(dc, rClient, GetXtremeColor(COLOR_WINDOW));
	OnDraw(&memDC);
	
	if (!m_bAnimationFinished)
	{
		// create animation device context.
		CXTPAnimationDC animationDC(dc, rClient, GetXtremeColor(COLOR_WINDOW));
		OnDraw(&animationDC);
...
void AFX_CDECL CAnimateStatic::CustomAnimation(CRect rc, CDC* pDestDC, CDC* pSrcDC, int nType, int nSteps, int nAnimationTime)
{
	...
	case animateCircles: 
		{
			CXTPBufferDC backDC(pDestDC->GetSafeHdc(), rc, (COLORREF)-1L);
			backDC.TakeSnapshot();  // copy image from screen
	...
	default:
		CXTPAnimationDC::DefaultAnimation(rc, pDestDC, pSrcDC, nType, nSteps, nAnimationTime);
	}
}
