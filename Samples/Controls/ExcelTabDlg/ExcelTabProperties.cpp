...
void CExcelTabProperties::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_XTPColorPicker(pDX, IDC_CLR_TEXT, m_crText);
	DDX_XTPColorPicker(pDX, IDC_CLR_BACK, m_crBack);
	DDX_XTPColorPicker(pDX, IDC_CLR_SELTEXT, m_crSelText);
	DDX_XTPColorPicker(pDX, IDC_CLR_SELBACK, m_crSelBack);
}
...
BEGIN_MESSAGE_MAP(CExcelTabProperties, CDialog)
	...
	ON_CPN_XTP_SELENDOK(IDC_CLR_SELTEXT, OnSelEndOkSelTextClr)
	ON_CPN_XTP_SELENDOK(IDC_CLR_SELBACK, OnSelEndOkSelBackClr)
END_MESSAGE_MAP()
...
void CExcelTabProperties::UpdateDlgStatus()
{
	...
	m_cpSelText.EnableWindow(bHasTabs);
	m_cpText.EnableWindow(bHasTabs);
	m_cpSelBack.EnableWindow(bHasTabs);
	m_cpBack.EnableWindow(bHasTabs);
}
...
void CExcelTabProperties::OnChangeTabNum() 
{
	...
	if (m_iTabNum < m_pExcelTabCtrl->GetItemCount())
	{
