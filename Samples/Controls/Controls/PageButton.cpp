...
BOOL CPageButton::OnInitDialog()
{
	...
	for (i = IDC_GBOX_THEME; i <= IDC_CHECK_ALPHA; i++)
		SetResize(i, CXTPResizePoint((float)(2.0/3.0), 0), XTP_ANCHOR_TOPRIGHT);
...
void CPageButton::OnDestroy()
{
	// Save window placement
	SavePlacement(_T("CPageButton"));

	CXTPResizePropertyPage::OnDestroy();
}

void CPageButton::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	{
		CXTPResizePropertyPage::OnSysCommand(nID, lParam);
	}
}
...
HCURSOR CPageButton::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}


void CPageButton::OnRadTheme() 
{
	UpdateData();

	m_btnRun.SetTheme((XTPControlTheme)m_nTheme);
	m_btnFind.SetTheme((XTPControlTheme)m_nTheme);
	m_btnSettings.SetTheme((XTPControlTheme)m_nTheme);
	m_btnHelp.SetTheme((XTPControlTheme)m_nTheme);

	m_btnState1.SetTheme((XTPControlTheme)m_nTheme);
	m_btnState2.SetTheme((XTPControlTheme)m_nTheme);
	m_btnOption1.SetTheme((XTPControlTheme)m_nTheme);
	m_btnOption2.SetTheme((XTPControlTheme)m_nTheme);

	m_frmRegular.SetTheme((XTPControlTheme)m_nTheme);
	m_frmTwoRows.SetTheme((XTPControlTheme)m_nTheme);
	m_frmCheckboxes.SetTheme((XTPControlTheme)m_nTheme);
	m_frmRadio.SetTheme((XTPControlTheme)m_nTheme);
	m_frmJustification.SetTheme((XTPControlTheme)m_nTheme);

	RedrawWindow(0, 0, RDW_ALLCHILDREN|RDW_INVALIDATE|RDW_ERASE);

	m_bUseVisualStyle = m_btnRun.GetUseVisualStyle();
	UpdateData(FALSE);
