...
void CTipWindowDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_XTPColorPicker(pDX, IDC_CLR_TEXT, m_clrTipText);
	DDX_XTPColorPicker(pDX, IDC_CLR_BACK, m_clrTipBack);
}
...
BEGIN_MESSAGE_MAP(CTipWindowDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_CPN_XTP_SELENDOK(IDC_CLR_TEXT, OnSelEndOkTextColor)
	ON_CPN_XTP_SELENDOK(IDC_CLR_BACK, OnSelEndOkBackColor)
END_MESSAGE_MAP()
...
void CTipWindowDlg::UpdateStyle()
{
	...
	if (m_bAlphaShadow) {
		m_dwTipStyle |= TWS_XTP_ALPHASHADOW;
	}
	if (m_bThickBorder) {
		m_dwTipStyle |= TWS_XTP_THICKBORDER;
	}
	if (m_bDropShadow) {
		m_dwTipStyle |= TWS_XTP_DROPSHADOW;
	}
}
