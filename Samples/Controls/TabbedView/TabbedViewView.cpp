...
IMPLEMENT_DYNCREATE(CTabbedViewView, CXTPTabView)

BEGIN_MESSAGE_MAP(CTabbedViewView, CXTPTabView)
...
int CTabbedViewView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	SetTheme((XTPControlTheme) ((CMainFrame*)AfxGetMainWnd())->m_iTheme);
...
void CTabbedViewView::OnTimer(UINT_PTR nIDEvent) 
{
    ...
    {
        CXTPTabView::OnTimer(nIDEvent);
    }
}

void CTabbedViewView::OnMouseMove(UINT nFlags, CPoint point) 
{
	CXTPTabView::OnMouseMove(nFlags, point);
...
{
	BOOL bIsOfficeTheme = ((CMainFrame*)AfxGetMainWnd())->m_iTheme == xtpControlThemeOfficeXP;
...
DROPEFFECT CTabbedViewView::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point) 
{
    ...
    return CXTPTabView::OnDragOver(pDataObject, dwKeyState, point);
}
...
void CTabbedViewView::OnDropFiles(HDROP hDropInfo) 
{
	...
	CXTPTabView::OnDropFiles(hDropInfo);
}
