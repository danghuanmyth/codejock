...
BEGIN_MESSAGE_MAP(CEditListBoxEx, CXTPEditListBox)
...
void CEditListBoxEx::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos)
{
	...
	CXTPListBox::OnWindowPosChanging(lpwndpos);
}
...
CTabCtrlDlg::CTabCtrlDlg(CWnd* pParent /*=NULL*/)
	: CXTPResizeDialog(CTabCtrlDlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CTabCtrlDlg, CXTPResizeDialog)
...
BOOL CTabCtrlDlg::OnInitDialog()
{
	...
	SetResize(IDC_TAB_CTRL,     XTP_ANCHOR_TOPLEFT,    XTP_ANCHOR_BOTTOMRIGHT);
	SetResize(IDOK,             XTP_ANCHOR_TOPRIGHT,   XTP_ANCHOR_TOPRIGHT);
	SetResize(IDCANCEL,         XTP_ANCHOR_TOPRIGHT,   XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_AUTOCONDENSE, XTP_ANCHOR_BOTTOMLEFT, XTP_ANCHOR_BOTTOMLEFT);
	SetResize(IDC_VERTICAL,     XTP_ANCHOR_BOTTOMLEFT, XTP_ANCHOR_BOTTOMLEFT);
	SetResize(IDC_MULTILINE,    XTP_ANCHOR_BOTTOMLEFT, XTP_ANCHOR_BOTTOMLEFT);
	SetResize(IDC_XPMODE,       XTP_ANCHOR_BOTTOMLEFT, XTP_ANCHOR_BOTTOMLEFT);
	SetResize(IDC_ARROWS,       XTP_ANCHOR_BOTTOMLEFT, XTP_ANCHOR_BOTTOMLEFT);
	SetResize(IDC_CLOSE,        XTP_ANCHOR_BOTTOMLEFT, XTP_ANCHOR_BOTTOMLEFT);
...
int CTabCtrlDlg::InitEditList()
{
	...
	m_listBox.SetFont (&XTPAuxData().font);
...
void CTabCtrlDlg::OnXpmode()
{
	...
	m_tabCtrl.SetTheme(m_bXPMode ? xtpControlThemeOfficeXP : xtpControlThemeDefault);

	if ( m_bXPMode )
	{
		m_listBox.GetEditGroup().SetButtonTheme(xtpControlThemeOfficeXP);
		m_listBox.GetEditGroup().SetWindowPos(NULL, 0,0,0,0, SWP_NOMOVE|SWP_NOSIZE|SWP_FRAMECHANGED);
	}
	else
	{
		m_listBox.GetEditGroup().SetButtonTheme(xtpControlThemeDefault);
		m_listBox.GetEditGroup().SetWindowPos(NULL, 0,0,0,0, SWP_NOMOVE|SWP_NOSIZE|SWP_FRAMECHANGED);
	}
...
void CTabCtrlDlg::OnChkButtons()
{
	...
	m_tabCtrl.ShowNavButtons((m_bCloseButton? xtpNavBtnClose: 0) |
		(m_bArrowButtons? xtpNavBtnArrows: 0));
...
void CTabCtrlDlg::OnSelChangeTab(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	CXTPTabCtrl* pTabCtrl = DYNAMIC_DOWNCAST(CXTPTabCtrl, CWnd::FromHandle(pNMHDR->hwndFrom));
...
IMPLEMENT_DYNAMIC(CTabCtrlEx, CXTPTabCtrl)

BEGIN_MESSAGE_MAP(CTabCtrlEx, CXTPTabCtrl)
	//{{AFX_MSG_MAP(CTabCtrlEx)
	ON_COMMAND(XTP_IDC_TAB_CLOSE, OnCloseTab)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
