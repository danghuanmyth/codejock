...
void CPageDragAndDropClient::OnPaint()
{
	...
	if (m_arrItems.GetSize() == 0)
	{
		...
		CXTPFontDC font(&dc, GetTaskPanel()->GetPaintManager()->GetItemFont(0));
		dc.DrawText(_T("Drag and drop task panel items to or from this area.\nYou can also throw items away in the trash can."), rcText, DT_TOP | DT_WORDBREAK);
	}
