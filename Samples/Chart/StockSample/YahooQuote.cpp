...
LPSTR ReadStringA(CStdioFile *pFile, LPSTR pszString, UINT nMax)
{
	CHAR chBuffer; // Read buffer
	UINT nRead = 0;
	BOOL bEOL = FALSE;

	while(!bEOL && sizeof(chBuffer) == pFile->Read(&chBuffer, sizeof(chBuffer)))
	{
		if (10 == chBuffer)
		{
			bEOL = TRUE;
...
CStringArray& CYahooQuote::GetHistory(CString strSym, CTime timeStart, CTime timeEnd, char ch /*='d'*/)
{
	...
	if (pFile)
	{
		...
		while (ReadStringA(pFile, szBuffer, _countof(szBuffer)))
		{
			CString strRead = szBuffer;
			m_strArray.Add(strRead);
		}
