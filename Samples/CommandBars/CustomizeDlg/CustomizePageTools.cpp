...
IMPLEMENT_DYNCREATE(CCustomizePageTools, CXTPPropertyPage)

CCustomizePageTools::CCustomizePageTools() : CXTPPropertyPage(CCustomizePageTools::IDD)
{
...
BEGIN_MESSAGE_MAP(CCustomizePageTools, CXTPPropertyPage)
	...
	ON_LBN_XTP_LABELEDITEND(IDC_LBOX_TOOLSLIST, OnLabelEditEnd)
	ON_LBN_XTP_LABELEDITCANCEL(IDC_LBOX_TOOLSLIST, OnLabelEditCancel)
	ON_LBN_XTP_NEWITEM(IDC_LBOX_TOOLSLIST, OnNewItem)
	ON_LBN_XTP_PREDELETEITEM(IDC_LBOX_TOOLSLIST, OnPreDeleteItem)
END_MESSAGE_MAP()
...
BOOL CCustomizePageTools::OnInitDialog()
{
	...
	m_editToolsCmd.Initialize(this, BES_XTP_CHOOSEFILE);
	...
	m_lboxToolsList.SetListEditStyle( _T(" &Menu contents:"), LBS_XTP_DEFAULT);

	int iArgPopup = 0; //IDR_ARG_POPUP;
	int iDirPopup = 0;//IDR_DIR_POPUP;

	if ( iArgPopup != 0 ) {
		m_editToolsArg.Initialize(this, BES_XTP_POPUPMENU, iArgPopup, 0 );
	}
	else {
		m_editToolsArg.Initialize(this, BES_XTP_CHOOSEFILE );
	}

	if ( iDirPopup != 0 ) {
		m_editToolsDir.Initialize(this, BES_XTP_POPUPMENU, iDirPopup, 0 );
	}
	else {
		m_editToolsDir.Initialize(this, BES_XTP_CHOOSEDIR );
	}
