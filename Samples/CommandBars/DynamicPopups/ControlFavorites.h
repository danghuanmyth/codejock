...
class CControlShellFile : public CXTPControlButton, CXTPShellPidl
{
public:
	CControlShellFile(XTP_TVITEMDATA* lptvid);
	void OnExecute();

public:
	XTP_TVITEMDATA* m_lptvid;
};



class CControlShell : public CXTPControlButton, CXTPShellPidl
{
public:
	CControlShell();
	CControlShell(XTP_TVITEMDATA* lptvid);

	virtual void OnCalcDynamicSize(DWORD dwMode);
	virtual BOOL IsCustomizeDragOverAvail(CXTPCommandBar* pCommandBar, CPoint point, DROPEFFECT& dropEffect);
	
	void InitTreeViewItems(LPSHELLFOLDER lpsf, LPCITEMIDLIST lpifq, CArray<XTP_TVITEMDATA*, XTP_TVITEMDATA*>& items);
