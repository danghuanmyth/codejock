...
CControlShellFile::CControlShellFile(XTP_TVITEMDATA* lptvid)
{
	m_lptvid = lptvid;
}
...
CControlShell::CControlShell(XTP_TVITEMDATA* lptvid)
{	
	m_lptvid = lptvid;
...
void CControlShell::InitTreeViewItems(LPSHELLFOLDER lpsf, LPCITEMIDLIST lpifq, CArray<XTP_TVITEMDATA*, XTP_TVITEMDATA*>& items)
{
	...
	if (SUCCEEDED(lpsf->EnumObjects(AfxGetMainWnd()->GetSafeHwnd(), SHCONTF_FOLDERS | SHCONTF_NONFOLDERS, &lpe)))
	{
		...
		while (lpe->Next(1, &lpi, &ulFetched) == S_OK)
		{
			...
			XTP_TVITEMDATA* lptvid = (XTP_TVITEMDATA*)lpMalloc.Alloc(sizeof(XTP_TVITEMDATA));
			if (lptvid == NULL)
			{
				if (lpe) {
					lpe->Release();
...
void CControlShell::OnCalcDynamicSize(DWORD /*dwMode*/)
{
	...
	CArray<XTP_TVITEMDATA*, XTP_TVITEMDATA*> items;
	InitTreeViewItems(lpsf, lpifq, items);
	qsort(items.GetData(), items.GetSize(), sizeof(XTP_TVITEMDATA*), TreeViewCompareProc2);

	for (int nIndex = 0; nIndex < items.GetSize(); nIndex++)
	{
		XTP_TVITEMDATA* lptvid = items[nIndex];
