...
void CMainFrame::SetCustomTheme(CXTPPaintManager* pTheme)
{
	...
	if ((m_bOffice2007Frame == 1) || ((pTheme->BaseTheme() == xtpThemeResource || pTheme->BaseTheme() == xtpThemeRibbon) &&
		(m_bOffice2007Frame == 2)))
	{
	...
	CXTPPaintManager::SetTheme(pTheme->BaseTheme());
...
void CMainFrame::OnCustomize()
{
	...
	//CXTPPropertyPageListNavigator* pNavigator = new CXTPPropertyPageListNavigator();
	//pNavigator->SetTheme(xtpControlThemeResource);
	//cs.SetNavigator(pNavigator);
	//cs.SetPageBorderStyle(xtpPageBorderNone);
...
int CMainFrame::OnCreateControl(LPCREATECONTROLSTRUCT lpCreateControl)
{
	if (lpCreateControl->bToolBar == FALSE)
	{
		...
		if (lpCreateControl->nID == ID_PARA_SEARCH)
		{
			CXTPControlEdit* pControl = new CXTPControlEdit();
			pControl->SetWidth(133);
			pControl->ShowBuddyButton(ID_APP_ABOUT);
